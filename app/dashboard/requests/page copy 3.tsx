...
export default function RequestsPage() {
        ...
        setRequests((prev) =>
          prev.map((request) => 
            request._id === selectedRequest._id 
              ? { 
                  ...request,
                  ...Object.entries(editFormData).reduce((acc, [key, value]) => {
                    if (key === 'targetDepartments' && Array.isArray(value)) {
                      acc[key] = (value as string[]).map(deptId => ({
                        _id: deptId,
                        name: '', // These should be properly populated from the department data
                        description: ''
  ...
  const openEditDialog = (request: RequestItem) => {
    ...
    setEditFormData({
      ...
      targetDepartments: request.targetDepartments.map(dep => dep._id),
      assignedDirectors: request.assignedDirectors.map(d => d._id).join(','),
            ...
            <div className="space-y-4 sm:space-y-6">
              ...
              <div className="flex items-center space-x-3 p-3 rounded-lg bg-slate-50">
                <Checkbox
                  ...
                  onCheckedChange={(checked) =>
                    setEditFormData({ ...editFormData, requiresSignature: checked as boolean })
                  }
                  className={`${themeColors.accent}`}
                />
                <Label htmlFor="edit-requires-signature" className="text-sm font-medium cursor-pointer">
              ...
              <div className="space-y-2">
                ...
                <div className="border-2 border-dashed rounded-lg p-4">
                  {editFormData.targetDepartments.map((deptId) => {
                      ...
                      <Badge key={deptId} className={`${themeColors.badgeSecondary} text-xs`}>
                        ...
                        <Button
                          ...
                          onClick={() =>
                            setEditFormData({
                              ...editFormData,
                              targetDepartments: editFormData.targetDepartments.filter((id) => id !== deptId),
                            })
                          }
                        >
                          <X className="w-3 h-3" />
              ...
              <div className="space-y-2">
                ...
                <Select
                  value={editFormData.assignedDirectors}
                  onValueChange={(value) => setEditFormData({ ...editFormData, assignedDirectors: value })}
                >
                  <SelectTrigger className={`${themeColors.focus}`}>
