...
export default function FileUploadForm({ onClose, onSuccess, authContext, themeColors }: FileUploadFormProps) {
  ...
  const handleFileSelect = (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    // Validate file size (15MB limit to match backend)
  ...
  const handleSubmit = async (action: "draft" | "submit") => {
    ...
    try {
      ...
      formDataToSend.append("fileSize", String(selectedFile.size))
      ...
      } else if (error.message?.includes("File size exceeds 15MB limit")) {
        errorMessage = "File size is too large. Maximum allowed size is 15MB"
        setError("The selected file exceeds the maximum allowed size of 15MB")
